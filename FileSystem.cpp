...
FileSystem::FileSystem(Application* app, bool start_enabled, const char* game_path) : Module(app, true)
{
	...
	// Dump list of paths
	App->log->AddLog("FileSystem Operations base is [%s] plus:", GetBasePath());
	App->log->AddLog(GetReadPaths());

	// enable us to write in the game's dir area
	if (PHYSFS_setWriteDir(".") == 0)
		App->log->AddLog("File System error while creating write dir: %s\n", PHYSFS_getLastError());
...
{
	App->log->AddLog("Loading File System\n");
	bool ret = true;

	// Ask SDL for a write dir
	char* write_path = SDL_GetPrefPath(App->orgName.c_str(), App->appTitle.c_str());
...
bool FileSystem::AddPath(const char* path_or_zip)
{
	...
	if (PHYSFS_mount(path_or_zip, nullptr, 1) == 0)
		App->log->AddLog("File System error while adding a path or zip: %s\n", PHYSFS_getLastError());
	else
		ret = true;
...
bool FileSystem::CopyFromOutsideFS(const char* full_path, const char* destination)
{
	...
	if (source && dest)
	{
		...
		App->log->AddLog("File System copied file [%s] to [%s]\n", full_path, destination);
	}
	else
		App->log->AddLog("File System error while copy from [%s] to [%s]\n", full_path, destination);
	...
	if (fs_file != nullptr)
	{
		...
		if (size > 0)
		{
			*buffer = new char[size];
			...
			if (readed != size)
			{
				App->log->AddLog("File System error while reading from file %s: %s\n", file, PHYSFS_getLastError());
				RELEASE(buffer);
			}
			else
				ret = readed;
		...
		if (PHYSFS_close(fs_file) == 0)
			App->log->AddLog("File System error while closing file %s: %s\n", file, PHYSFS_getLastError());
	}
	else
		App->log->AddLog("File System error while opening file %s: %s\n", file, PHYSFS_getLastError());
...
uint FileSystem::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	...
	if (fs_file != nullptr)
	{
		...
		if (written != size)
			App->log->AddLog("File System error while writing to file %s: %s\n", file, PHYSFS_getLastError());
		else
		{
			if (append == true)
				App->log->AddLog("Added %u data to [%s%s]\n", size, PHYSFS_getWriteDir(), file);
			//else if(overwrite == true)
				//LOG("File [%s%s] overwritten with %u bytes", PHYSFS_getWriteDir(), file, size);
			else if (overwrite == false)
				App->log->AddLog("New file created [%s%s] of %u bytes\n", PHYSFS_getWriteDir(), file, size);
...
bool FileSystem::Remove(const char* file)
{
	...
	if (file != nullptr)
	{
		...
		{
			App->log->AddLog("File deleted: [%s]\n", file);
			ret = true;
		}
		else
			App->log->AddLog("File System error while trying to delete [%s]: \n", file, PHYSFS_getLastError());
	}
...
size_t AssimpWrite(aiFile* file, const char* data, size_t size, size_t chunks)
{
	...
	if (ret == -1)
		MYLOG("File System error while WRITE via assimp: %s\n", PHYSFS_getLastError());
...
size_t AssimpRead(aiFile* file, char* data, size_t size, size_t chunks)
{
	...
	if (ret == -1)
		MYLOG("File System error while READ via assimp: %s\n", PHYSFS_getLastError());
...
size_t AssimpTell(aiFile* file)
{
	...
	if (ret == -1)
		MYLOG("File System error while TELL via assimp: %s\n", PHYSFS_getLastError());
...
size_t AssimpSize(aiFile* file)
{
	...
	if (ret == -1)
		MYLOG("File System error while SIZE via assimp: %s\n", PHYSFS_getLastError());
...
void AssimpFlush(aiFile* file)
{
	if (PHYSFS_flush((PHYSFS_File*)file->UserData) == 0)
		MYLOG("File System error while FLUSH via assimp: %s\n", PHYSFS_getLastError());
}
...
aiReturn AssimpSeek(aiFile* file, size_t pos, aiOrigin from)
{
	...
	if (res == 0)
		MYLOG("File System error while SEEK via assimp: %s\n", PHYSFS_getLastError());
...
void AssimpClose(aiFileIO* io, aiFile* file)
{
	if (PHYSFS_close((PHYSFS_File*)file->UserData) == 0)
		MYLOG("File System error while CLOSE via assimp: %s\n", PHYSFS_getLastError());
}
